...
Because it reads only the body, an edit that drops the `## Pipeline` section the pipeline wrote turns a passing check red on an unchanged head commit; keep that section when you rewrite a PR body, and restore it to make the check pass again.
...
bin/fm-test-run.sh --lane portable-serial   # local serial remainder union (watcher/AFK/tmux/stateful)
bin/fm-test-run.sh --lane portable-serial-1 # CI serial shard 1 (LPT half of the remainder)
bin/fm-test-run.sh --lane portable-serial-2 # CI serial shard 2
bin/fm-test-run.sh --check-coverage   # prove portable shards + serial shards + Herdr equal the full inventory
...
A registry row may only add what the runner leaves open: a family row must name a listed family and a script the runner leaves unclassified, and a covers row must name a path the runner does not own, and the runner refuses any row that would instead replace an upstream answer, so an adds row is how a fork contributes an owner for a path the runner does own and upstream's answer survives alongside it.
...
CI owns broad regression across required portable parallel shards, the two portable serial lanes, the Herdr lane, lint, invariants, the coverage guard, and stock macOS Bash compatibility in [`.github/workflows/ci.yml`](.github/workflows/ci.yml).
...
The same self-skip rule covers host-capability gates: the tracked TypeScript extension tests skip themselves on a node runtime that cannot import TypeScript, and each gate's own comment in the test names the runtime it requires.
