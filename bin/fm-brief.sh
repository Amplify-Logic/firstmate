...
if [ -n "${FM_DATA_OVERRIDE:-}" ]; then
  DATA=$(fm_path_require_directory FM_DATA_OVERRIDE "$FM_DATA_OVERRIDE") || exit 1
else
  DATA="$FM_HOME/data"
fi
if [ -n "${FM_STATE_OVERRIDE:-}" ]; then
  STATE=$(fm_path_require_directory FM_STATE_OVERRIDE "$FM_STATE_OVERRIDE") || exit 1
else
  STATE="$FM_HOME/state"
fi
KIND=ship
...
'A named non-`default` session plus a `--session <name>` the helper places on every call is the only viable local isolation.' \
'' \
'1. Set `HERDR_LAB_HELPER='"$HERDR_LAB_HELPER"'` and generate the session name with `HERDR_LAB_SESSION=$("$HERDR_LAB_HELPER" name '"$ID"')`.' \
'   Install `trap '\''"$HERDR_LAB_HELPER" teardown "$HERDR_LAB_SESSION"'\'' EXIT` before provisioning, then provision only with `"$HERDR_LAB_HELPER" provision "$HERDR_LAB_SESSION"`.' \
'2. Run every task-specific non-lifecycle Herdr command through `"$HERDR_LAB_HELPER" run "$HERDR_LAB_SESSION" <arguments...>`.' \
'   The helper places the required `--session "$HERDR_LAB_SESSION"` on every call, before any child-argv `--` delimiter; `HERDR_SESSION` alone is never accepted as isolation.' \
...
# read -r -d '' preserves the heredoc's trailing newline that command
# substitution used to strip, so remove it to keep generated briefs stable.
DOD=${DOD%$'\n'}
