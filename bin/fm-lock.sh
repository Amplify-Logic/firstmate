...
# A live holder is judged by the fork's fm_harness_holder_alive
# (bin/fm-primary-scope-lib.sh), which adds a loose argv tier so a live primary
# the strict matcher misses is never taken over as stale.
#
...
# The fork's primary-scope predicates layer on that owner and add the loose
# live-holder tier (fm_harness_holder_alive) used below to judge a live owner.
...
if [ "${1:-}" = "status" ]; then
  ...
  # The fork's loose live-holder tier (bin/fm-primary-scope-lib.sh) also reads a
  # live harness the strict matcher misses as the holder, so status never calls
  # a live primary's lock stale.
  if [ "$FM_LOCK_INSPECT_STATE" = unknown ] && [ -n "$FM_LOCK_INSPECT_PID" ] \
    && fm_harness_holder_alive "$FM_LOCK_INSPECT_PID"; then
