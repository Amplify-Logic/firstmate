#!/usr/bin/env bash
# Install and inspect the local continuous channel-intake schedule on macOS launchd.
#
# Usage:
#   fm-channel-intake-schedule.sh render
#   fm-channel-intake-schedule.sh install
#   fm-channel-intake-schedule.sh status
#   fm-channel-intake-schedule.sh remove
#   fm-channel-intake-schedule.sh --help
#
...
#
# `StartInterval` plus `RunAtLoad` is the whole trigger, exactly as the morning
# gate's schedule uses it. launchd runs the job on login and then on the
# interval, and a job whose interval elapsed while the machine was asleep runs
# shortly after it wakes. THAT IS THE CATCH-UP PATH AND ITS ONLY GUARANTEE: a
# tick reads forward from each source's own checkpoint, so a sleep or offline
# gap is walked rather than skipped, but the interval is a target detection
# latency and never an upper bound. Nothing here makes the laptop poll while it
# is asleep and no 24x7 availability is claimed.
#
# The gate itself lives in bin/fm-channel-intake.sh, which owns the cadence,
# the per-source checkpoints, deduplication, backoff and the ledger. This
...
# agent and installing here never disturbs another home's schedule, including
# the morning gate's own agent, which uses a different stem.
set -eu
...
INTAKE="$SCRIPT_DIR/fm-channel-intake.sh"
...
die() {
  printf 'fm-channel-intake-schedule: %s\n' "$*" >&2
  exit 2
}
...
FM_LAUNCHD_STEM=channel-intake
FM_LAUNCHD_PROGRAM="$INTAKE"
FM_LAUNCHD_PROGRAM_ARG=tick
FM_LAUNCHD_ROOT="$ROOT"
FM_LAUNCHD_FM_HOME="$FM_HOME"
FM_LAUNCHD_LOG_DIR="$DATA/channel-intake"
FM_LAUNCHD_AGENTS_DIR=${FM_CHANNEL_INTAKE_LAUNCH_AGENTS_DIR:-$HOME/Library/LaunchAgents}
...
FM_LAUNCHD_LAUNCHCTL=${FM_CHANNEL_INTAKE_LAUNCHCTL:-launchctl}
...
fm_launchd_preinstall() {
  [ "$(intake status | awk '$1 == "enabled:" { print $2 }')" = true ] \
    || die "this home is not opted in; add 'enabled = true' to config/channel-intake first"
}

# Arm the live-session delivery path too, so a running fleet is actually woken
# instead of only discovering due sources at the next session start.
fm_launchd_postinstall() {
  intake arm-check
}

fm_launchd_postremove() {
  # Durable records under data/channel-intake are deliberately left in place:
  # removing the schedule is not the same as discarding the ledger, and an
  # open obligation must survive an uninstall.
