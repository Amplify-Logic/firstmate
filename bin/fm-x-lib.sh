...
# The pre-migration (v1) shim's exact bytes. The mode-0755 shim the watcher used
# to execute directly is recognized here only so bin/fm-pr-check-migrate.sh can
# identify and replace it; nothing generates it any more.
