...
# `enter` wrote before they launch an away daemon (a quiet entry needs none; see
# FM_AFK_MODE below).
...
# the ordinary supervision session runs in both postures. Quiet mode exists
# only as that daemon, so it is unavailable on Pi; on a supervision-host home
# quiet mode still runs the daemon, so a quiet entry or a refresh of a running
