...
# Usage: fm-harness.sh marker           print the environment-marker verdict alone,
#                                       before ancestry arbitration; empty when no
#                                       marker is present. The launch-boundary
#                                       identity bin/fm-primary.sh establishes is
#                                       only observable here, because detect_own
#                                       lets a contradicting harness ancestor win.
...
harness_marker() {
  ...
  # PI_CODING_AGENT arm below or a prime-agent worker misreports as pi and gets
  # steered with pi's vocabulary. Like cursor it does not clear an inherited
  # CLAUDECODE either, so it also precedes the claude marker. Every variable
  # here is set only by prime-agent itself or by Firstmate's own launch wrapper
  # (verified 2026-08-06, v0.7.0; data/fm-prime-agent-trial-t1).
...
harness_process_verdict() {  # <pid>
  ...
  case "$(basename -- "$comm")" in
    ...
    *grok*) echo "comm grok"; return ;;
    ...
    # prime-agent precedes the pi arms below: its CLI sets process.title to the
    # bare word `prime-agent` (verified 2026-08-07, v0.7.0), and its pi-fork
    # ancestry would otherwise trip the pi patterns. Anchored, never *prime*.
    ...
    node*|python*)
      ...
      case "$args" in
        # prime-agent's CLI rewrites process.title, so its node tree carries the
        # prime-agent launcher/script path in argv. Matched before *claude* and
        # before the bare pi patterns its pi-fork ancestry could otherwise trip.
        *prime-agent*) echo "args prime-agent"; return ;;
        *claude*) echo "args claude"; return ;;
        *codex*) echo "args codex"; return ;;
        *opencode*) echo "args opencode"; return ;;
        *grok*) echo "args grok"; return ;;
        *kimi*) echo "args kimi"; return ;;
