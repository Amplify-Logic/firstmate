...
# export, and the FM_HERDR_PROJECT_* pin). Zellij has no single-call atomic
# "run and submit" action, so this composes paste (literal) + send-keys
# Enter, exactly like send_literal + send_key are composed elsewhere - the
# two-step form is the ONLY form for this adapter, unlike tmux/herdr which
# have a genuinely atomic primitive.
