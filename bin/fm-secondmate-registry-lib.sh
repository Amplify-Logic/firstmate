...
# expected-home, active-home, and code-root arguments after its registry and
# path-resolver arguments.
# The last two adapt the common parser to this fork's isolated-home model by
# refusing a registered home equal to, inside, or containing either active root.
...
secondmate_registry_validate_bindings() {
  ...
  local active_home=${5:-} code_root=${6:-}
  local tmp snapshot bindings line id home home_key duplicate_homes duplicate_ids overlaps
  local expected_home_key active_key='' root_key=''
  SECONDMATE_REGISTRY_MATCH_HOME=
  SECONDMATE_REGISTRY_MATCH_HOME_KEY=
  SECONDMATE_REGISTRY_MATCH_PROJECTS=
  SECONDMATE_REGISTRY_ERROR=
