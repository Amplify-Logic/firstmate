...
# an open expectation for its one recovery request.
...
#
# Every confirmed text submit to a meta-resolved task target appends one line to
...
if [ "${1:-}" = "--key" ]; then
  ...
  # Confirmed submit: record one steer line for meta-resolved task targets so
  ...
  if [ -n "$PENDING_REPLY_CORR" ]; then
    if fm_pending_reply_confirm_delivery "$STATE" "$PENDING_REPLY_CORR"; then
      ...
      if [ "$delivery_commit_status" = 2 ]; then
        echo "error: text was delivered to $T, but its pending-reply record could not be finalized; a durable recovery marker was stored. Do not resend." >&2
      else
        echo "error: text was delivered to $T, but its pending-reply record and recovery marker both failed. Do not resend; inspect $STATE." >&2
      fi
      exit 1
    fi
  fi
