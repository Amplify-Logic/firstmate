...
# The physical spelling of a path, for identity comparisons. A directory
# resolves through `pwd -P`; anything else resolves its directory that way and
# keeps its own name (fm_lock_abs_path).
fm_canonical_path() {
  local path=$1
  [ -n "$path" ] || return 1
  if [ -d "$path" ]; then
