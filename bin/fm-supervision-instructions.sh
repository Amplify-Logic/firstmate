...
case "$HARNESS" in
  claude|codex|opencode|pi|grok|kimi|cursor) SNIPPET="$DOC_DIR/$HARNESS.md" ;;
...
repair_line() {
  if [ "$READ_ONLY" -eq 1 ]; then
    printf '%s\n' 'Watcher repair requires holding the fleet lock; do not drain, arm, or repair from this read-only session.'
    return 0
  fi
  if [ "$AFK" -eq 1 ]; then
