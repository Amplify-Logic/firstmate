...
fm_pending_reply_backend_observation() {  # <backend> <target> [expected-label]
  local backend=$1 target=$2 expected_label=${3-} agent_alive native tail40
  ...
  if printf '%s' "$tail40" | grep -v '^[[:space:]]*$' | tail -6 \
    | grep -qiE "${FM_BUSY_REGEX:-$FM_BUSY_REGEX_DEFAULT}"; then
    printf 'busy'
  else
...
fm_pending_reply_busy_state_from_observation() {  # <record-path> <observation>
  ...
  case "$observation" in
    busy|idle|unknown|dead) printf '%s' "$observation" ;;
    fallback-idle)
...
# Escalate a confidently dead secondmate agent once without sending a recovery
# request into its surviving shell. Session-start liveness remains the sole
...
fm_pending_reply_escalate_dead() {  # <state-dir> <corr_id>
  ...
  parent_status=$(fm_pending_reply_get "$rec" parent_status)
  [ -n "$parent_status" ] || return 1
  payload="pending-reply-endpoint-dead: task=${task_id} pending-reply-id=${corr} request=${summary}"
  mkdir -p "$(dirname "$parent_status")" 2>/dev/null || return 1
  if ! grep -Fqx "blocked: $payload" "$parent_status" 2>/dev/null; then
    printf 'blocked: %s\n' "$payload" >> "$parent_status" 2>/dev/null || return 1
  fi
...
# busy_state is busy|idle|unknown|dead for the secondmate endpoint.
...
fm_pending_reply_tick_one() {  # <state-dir> <corr_id> <busy_state> [secondmate-home]
  ...
  if [ "$busy_state" = dead ]; then
    fm_pending_reply_escalate_dead "$state" "$corr" 2>/dev/null || true
    return 0
  fi
  phase=$(fm_pending_reply_get "$rec" phase)
  case "$phase" in
