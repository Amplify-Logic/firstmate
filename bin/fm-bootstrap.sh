...
#        fm-bootstrap.sh manifest
#          Write the resolved tool->version set (backend-specific tools plus the
#          universal COMMON_TOOLS list) for environment-fidelity comparison
#          between machines. One `tool=version` line per tool; MISSING when
#          absent. Also prints backend=<name> and a generated= UTC stamp.
set -u
...
secondmate_sync() {
  # Source portable lock helpers only on the mutating path so detect-only startup
  # does not create state as a side effect of loading this script.
  ...
  while IFS='|' read -r id home _window _meta; do
    ...
    if ! reread_out=$(FM_HOME="$FM_HOME" FM_ROOT_OVERRIDE="$FM_ROOT" \
      FM_STATE_OVERRIDE="$STATE" FM_CONFIG_REREAD_SKIP_PENDING="$reread_skip_pending" \
...
crew_dispatch_validate() {
  ...
  err=$(jq -r '
    def verified($h): ["claude","codex","opencode","pi","grok","cursor","kimi","prime-agent"] | index($h);
    def effort_ok($h; $e):
      if $e == null then true
    ...
    elif [(.rules // [])[]? | .select? // empty | select(. != "quota-balanced" and . != "capability-recent")] | length > 0 then
      "unknown select: " + ([ (.rules // [])[]? | .select? // empty | select(. != "quota-balanced" and . != "capability-recent") ] | unique | join(", "))
