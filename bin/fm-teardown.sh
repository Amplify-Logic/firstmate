...
#   --legacy-record also reconciles the two degradations a record written before
...
TEARDOWN_ENDPOINT_UNBOUND=0
TEARDOWN_WORKTREE_ABSENT=0
...
TASK_TYPE=$(fm_meta_get "$META" task_type)
# The fork's capability-outcome log. Sourced behind a guard so an upstream copy
# without the library tears down exactly as it would have.
# shellcheck source=bin/fm-capability-lib.sh disable=SC1091
[ ! -r "$SCRIPT_DIR/fm-capability-lib.sh" ] || . "$SCRIPT_DIR/fm-capability-lib.sh"
...
# worktree is the correct working directory for that repo-scoped query and
# disappears below. Anything unavailable degrades to outcome=unknown - never
# guessed, never fatal (bin/fm-capability-lib.sh owns the outcome rules). The
# bound keeps very long run histories honest-but-bounded: only the newest 200
# recorded attempts are considered.
CAP_OUTCOME=
CAP_FIX_ROUNDS=
CAP_STEERS=
if command -v fm_capability_outcome_from_runs >/dev/null 2>&1 \
  && [ "$KIND" != secondmate ] && [ -n "$HARNESS" ]; then
  if [ "$FORCE" != "--force" ]; then
    CAP_OUTCOME=unknown
  fi
  CAP_BRANCH=
  ...
  if [ -n "$CAP_BRANCH" ] && [ "$FORCE" != "--force" ]; then
    CAP_RUNS=$( ( cd "$WT" && fm_run_timed 20 no-mistakes runs --limit 200 ) 2>/dev/null || true )
...
[ ! -e "$CONFIG/fleet-ledger" ] || FM_HOME=$FM_HOME FM_STATE_OVERRIDE=$STATE FM_CONFIG_OVERRIDE=$CONFIG "$SCRIPT_DIR/fm-fleet-ledger.sh" cleaned_up "$ID" || true
# Record capability evidence before meta disappears (ship/scout only;
# best-effort). The steer counter is a volatile state file read here.
...
command -v fm_capability_record_teardown >/dev/null 2>&1 \
...
rm -f "$STATE/$ID.turn-ended" "$STATE/$ID.progress" \
  ...
  "$STATE/.$ID.branch-outcome-index" "$STATE/$ID.steers" \
  "$STATE/.secondmate-relaunch-$ID" "$STATE/.secondmate-relaunch-bound-$ID" \
  "$STATE/$ID.prime-ext.ts"
rm -rf "$STATE/$ID.prime-agent-home"
...
if [ "$TEARDOWN_LEGACY_ACCEPTED" = 1 ]; then
  echo "teardown $ID complete (window ${T:-none}, worktree ${WT:-<none recorded>}, legacy record accepted without spawn_gen: endpoint $TEARDOWN_LEGACY_ENDPOINT, incarnation $TEARDOWN_META_SPAWN_GEN${TEARDOWN_RECONCILED_NOTE})"
elif teardown_owns_worktree; then
  echo "teardown $ID complete (window ${T:-none}, worktree ${WT:-<none recorded>}${TEARDOWN_RECONCILED_NOTE})"
else
