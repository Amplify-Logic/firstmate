...
# landed, rate-limit reset) and the recheck detects that. A captain-held
# transfer never re-surfaces on any cadence: only the captain's answer clears
# it, and nothing the recheck can observe changes. Live vs dead is told apart
# by fm_backend_agent_alive, never by the status verb: a HEALTHY idle pane
# absorbs completely silently (the decision is durably tracked), while a
...
handle_paused_stale() {  # <window> <task> <hash>
  local win=$1 task=$2 h=$3 key statusf mtime age rf rf_age reason last surf digest alive
  ...
  elif [ "$age" -ge "$PAUSE_RESURFACE_SECS" ] && [ "$rf_age" -ge "$PAUSE_RESURFACE_SECS" ] \
