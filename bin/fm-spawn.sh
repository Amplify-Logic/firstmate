...
# Usage: fm-spawn.sh <task-id> <project-dir> --mode <no-mistakes|direct-PR|local-only> --yolo <on|off> [--branch-prefix <prefix>] [--harness <name>|harness|launch-command] [--model <name>] [--effort <level>] [--backend <name>] [--account <name>] [--outcome <text>] [--task-type <slug>]
...
#   /updatefirstmate, restart). A bare adapter name (claude|codex|opencode|pi|pi-signed|grok|kimi|cursor|gemini|muse|rovo|omp|agy|devin|prime-agent)
...
[ "$OUTCOME_SET" -eq 0 ] || [ -n "$OUTCOME" ] || {
  echo "error: --outcome requires a non-empty value" >&2
  exit 1
}
[ "$TASK_TYPE_SET" -eq 0 ] || [ -n "$TASK_TYPE" ] || {
  echo "error: --task-type requires a non-empty value" >&2
  exit 1
}
if [ -n "$TASK_TYPE" ]; then
  case "$TASK_TYPE" in
  *'|'*)
    echo "error: --task-type must not contain '|'" >&2
    exit 1
    ;;
  esac
  case "$TASK_TYPE" in
  *$'\n'* | *$'\r'*)
    echo "error: --task-type must not contain newlines" >&2
    exit 1
    ;;
  esac
fi
...
if [ "$RELAUNCH" -eq 1 ]; then
  ...
  case "${POS[1]:-}" in
  '' | claude | codex | opencode | pi | pi-signed | grok | kimi | cursor | gemini | muse | rovo | omp | agy | devin | prime-agent)
    ARG3=${POS[1]:-}
    ;;
  *' '*)
...
launch_template() {
  ...
  codex)
    if [ "$kind" = secondmate ]; then
      ...
      printf '%s' 'codex __MODELFLAG____EFFORTFLAG__--dangerously-bypass-approvals-and-sandbox --disable hooks -c model_auto_compact_token_limit=500000 -c "notify=[\"bash\",\"-c\",\"touch __TURNEND__\"]" "$(__OPINPUT__ encode launch-brief < __BRIEF__)"'
    fi
    ;;
...
# A home's worker account pin for this runner (bin/fm-worker-account-lib.sh,
...
if [ -n "$WORKER_ACCOUNT" ]; then
  [ "$ACCOUNT_SET" -eq 0 ] || {
    echo "error: --account cannot be combined with this home's $HARNESS worker account pin (docs/configuration.md \"Worker account pin\"); drop --account or remove the pin" >&2
    exit 1
  }
elif command -v fm_account_spawn_pin >/dev/null 2>&1; then
...
EOF
  ...
  herdr)
    ...
    if [ "$KIND" != secondmate ] && fm_backend_herdr_presentation_enabled "$CONFIG" "$STATE"; then
      ...
      # Only the fresh-projection branch below names a parent, and it reads the
      # label that parent actually carries. The reclaim branch resolves its
      # parent by the id its journal recorded, so it needs no label at all.
      HERDR_PARENT_LABEL=
...
if [ "$RELAUNCH" -eq 1 ]; then
  ...
  for _ in $(seq 1 "$settle_max_polls"); do
    ...
    [ "$settle_sleep" -eq 0 ] || sleep "$settle_sleep"
  done
  if [ -z "$WT" ]; then
    echo "error: treehouse get did not enter an isolated worktree within ${settle_max_polls} polls (last seen '${last_seen:-none}': $last_reason; spawning project '$PROJ_ABS'); inspect window $T" >&2
    exit 1
  fi
...
# The launch substitutes the PHYSICAL state path: the daemon binds the
# socket at exactly the path it is handed, and teardown resolves the same
# physical form to find that listener again.
...
# A named vendor account (ACCOUNT_HOME above) never coexists with a pin for the
# same runner, because the fork's selection stands down under a pin.
if [ -n "$WORKER_ACCOUNT" ]; then
  case "$HARNESS" in
  claude)
    if [ -n "$WORKER_ACCOUNT_ROOT" ]; then
...
elif [ "$HARNESS" = claude ] && [ -z "$ACCOUNT_HOME" ] && [ -n "${CLAUDE_CONFIG_DIR:-}" ]; then
...
spawn_send_key "$T" Enter
# The fork's generic agent-up gate: prove a real agent process owns the pane
