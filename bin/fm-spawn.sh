...
#   /updatefirstmate, restart). A bare adapter name (claude|codex|opencode|pi|pi-signed|grok|kimi|cursor|gemini|muse|rovo|omp|agy|prime-agent)
...
if [ "$RELAUNCH" -eq 1 ]; then
  ...
  case "${POS[1]:-}" in
    ''|claude|codex|opencode|pi|pi-signed|grok|kimi|cursor|gemini|muse|rovo|omp|agy|prime-agent)
      ARG3=${POS[1]:-}
      ;;
    *' '*)
...
launch_template() {
  ...
  case "$harness" in
    ...
    claude)
      ...
      # reapable, so this stays scoped to kind=secondmate.
      if [ "$kind" = secondmate ]; then
