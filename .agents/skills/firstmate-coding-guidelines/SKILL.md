...
- The one exception is a capability declared in `fork-surface.conf` whose integration contract has no executable boundary that can prove it; its source assertion names that capability and is retired as soon as a behavioral boundary exists.
