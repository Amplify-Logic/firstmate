...
The same launch and the same locked bootstrap sweep also propagate the primary's declared inherited local material: `config/crew-dispatch.json`, `config/crew-harness`, `config/backlog-backend`, `config/backend`, `config/startup-memory-budget`, and the one shared captain-preference file `data/captain-shared.md`.
...
Inherited `config/backend` becomes that home's default worker runtime only for future work; it never moves, stops, or restarts work already running.
A present primary value converges byte-for-byte, and primary absence removes the inherited copy so that home returns to runtime auto-detection.
An explicit per-spawn `--backend` or `FM_BACKEND` remains stronger than the inherited default.
Inherited `config/startup-memory-budget` bounds the three startup memory files under the `/stow` contract, while the primary owns its validated value and default publication.
...
It uses the same live-home discovery and propagation helper as bootstrap, reports each item as `pushed`, `unchanged`, `skipped`, or `error`, and applies the reread contract above for changed or pending generations.
...
Run `bin/fm-home-seed.sh validate` when checking registry integrity; its header owns the complete parsing, isolated-home, and refusal mechanics.
...
When that meta records `account=`, pass it back as `--account <name>` so a pinned secondmate returns on the same vendor login; the locked session-start liveness sweep already carries it forward this way (`docs/configuration.md` "Vendor account pinning").
