...
Example: `../../../bin/fm-spawn.sh <task-id> <project> --scout --harness cursor --model cursor-grok-4.6-high`.
...
`FM_PRIMARY_HARNESS` is a launch-boundary marker read by `fm-harness.sh marker`; ancestry still arbitrates the final verdict, so a leaked value cannot rename an unrelated session.
