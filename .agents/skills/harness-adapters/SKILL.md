...
`cursor` is worker-only: primary-orchestrator support was never verified, so never launch a firstmate primary on it and never infer primary support from worker support.
