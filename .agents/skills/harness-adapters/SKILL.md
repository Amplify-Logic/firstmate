...
description: >-
  ...
  Contains verified facts for claude, codex, opencode, pi, pi-signed, grok, kimi, cursor, gemini, muse, rovo, omp, agy, devin, and this fork's prime-agent; also this fork's Cursor PRIMARY profile.
...
Muse, Gemini, AGY, Rovo, Devin, and prime-agent are verified only for crewmate and scout work, never a secondmate or primary.
...
Kimi establishes its unsupported primary boundary in its selected harness reference; this fork certifies a Cursor PRIMARY profile of its own through `bin/fm-primary.sh`, and each harness reference states which role its facts cover.
