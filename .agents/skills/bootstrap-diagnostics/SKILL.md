...
- `TOOLCHAIN_DRIFT: <runtime> installed <version>, certified <version> (<evidence>) - <why>` - an agent runtime on PATH is not the build this repo carries certification evidence for.
  ...
  A `kimi` line is not one of those: `bin/fm-primary.sh` warns on Kimi version drift and launches anyway, so the line means the running build is uncertified rather than down, and it batches like any other drift line.
