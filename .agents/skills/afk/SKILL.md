...
2. **Per harness, after the record exists (a quiet entry needs none; see the `quiet` skill):**
   ...
   Both daemon paths share `bin/fm-afk-start.sh` as the daemon entry; an away entry requires the record `enter` wrote, while a quiet entry needs none.
