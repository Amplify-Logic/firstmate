...
test_captain_visible_warnings_use_outcome_language() {
  ...
  assert_grep 'Translate every machine field, warning, and workflow label through `AGENTS.md` section 9 in both chat and file mode' "$BEARINGS_SKILL" \
    "file-mode Bearings can leak internal workflow labels"
  pass "Bearings translates inventory gaps and machine fields into outcome language"
}
