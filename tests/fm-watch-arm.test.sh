...
# Start the real watcher as the singleton holder. The case's own dir is pinned
# as FM_HOME and every spawned pid is tracked for cleanup reaping, so this
# watcher can never resolve a real operational home even if a future edit drops
# one of the explicit overrides.
start_seed_watcher() {  # <state> <fakebin> <watch-out>
  local state=$1 fakebin=$2 out=$3 i dir
  dir=$(dirname "$state")
  PATH="$fakebin:$PATH" FM_STATE_OVERRIDE="$state" FM_HOME="$dir" \
    FM_POLL=5 FM_SIGNAL_GRACE=1 \
    FM_CHECK_INTERVAL=999999 FM_HEARTBEAT=999999 "$WATCH" > "$out" &
  SEED_PID=$!
  fm_test_track_pid "$SEED_PID"
  i=0
  while [ "$i" -lt 60 ]; do
...
start_attached_arm() {  # <state> <fakebin> <arm-out> <confirm-timeout>
  local state=$1 fakebin=$2 armout=$3 confirm=$4 i dir
  dir=$(dirname "$state")
  PATH="$fakebin:$PATH" FM_STATE_OVERRIDE="$state" FM_HOME="$dir" \
