...
test_registry_home_with_pre_home_parentheses() {
  ...
  # Natural-language punctuation before and inside the structured suffix must not shift its fields.
  printf -- '- %s - issue triage (id is legacy); keep summary punctuation (home: %s; scope: issue triage (child); semicolon is meaningful; projects: alpha; added 2026-07-09)\n' \
    "$id" "$sub_abs" > "$home/data/secondmates.md"
