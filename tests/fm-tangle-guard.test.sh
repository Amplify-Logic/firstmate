...
test_guard_banner() {
  ...
  assert_contains "$out" "This read-only session must not restore the primary; restoring requires holding the fleet lock." "read-only guard did not explain restore ownership"
...
test_bootstrap_line() {
  ...
  assert_contains "$out" "this read-only session must not restore it; restoring requires holding the fleet lock" "detect-only bootstrap did not explain restore ownership"
