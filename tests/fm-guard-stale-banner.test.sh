...
# Record a genuinely live watcher holder for the case home: pid, home path,
# watcher path, and process identity, exactly as the real singleton writes them,
# so the guard's identity-matched health predicate has full evidence.
record_live_watcher() {
...
# The between-turns arm-owner model: the watcher runs only between turns, so a
# fresh beacon with no live watcher process is the healthy mid-turn state. This
# fork maps every harness to the persistent model by default; the override
# keeps the model branch covered and upstream-shaped.
...
test_autoarm_fresh_beacon_without_watcher_is_healthy() {
  ...
  # between-turns arm-owner model, where the watcher only runs between turns.
  touch "$(case_home "$dir")/state/.last-watcher-beat"
