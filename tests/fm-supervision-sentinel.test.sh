...
test_stale_beacon_alert_is_loud_deduplicated_backed_off_and_rearmed() {
  ...
  run_check "$home" "$recorder"
  [ "$(wc -l < "$log" | tr -d '[:space:]')" -eq 1 ] || fail "unchanged continuous outage emitted an early duplicate alert"

  touch -t 202001020000 "$home/state/.last-watcher-beat"
  run_check "$home" "$recorder"
  [ "$(wc -l < "$log" | tr -d '[:space:]')" -eq 2 ] || fail "new outage episode inherited the prior episode's backoff"
  marker="$home/state/.supervision-outage-alarm"
  assert_contains "$(cat "$marker")" 'delivery_count=1' "new outage episode did not reset the successful-delivery count"
