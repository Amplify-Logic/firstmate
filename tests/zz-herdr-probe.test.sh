...
# ---- TEMPORARY DIAGNOSTIC PROBE (not committed) ----
PROBE_OUT="$ROOT/.probe"
lab workspace list > "$PROBE_OUT/workspaces.json" 2>&1 || true
{
  echo "== FIRSTMATE_WSID=$FIRSTMATE_WSID  SECOND_WSID=$SECOND_WSID"
  for M in "$HOME_DIR/state/p1.meta" "$SECOND_HOME_A/state/a1.meta" \
           "$SECOND_HOME_A/state/a2.meta" "$SECOND_HOME_B/state/b1.meta"; do
    echo "== META $M"; grep -E '^(herdr_|outcome=|kind=)' "$M" 2>&1 || true
  done
  for E in a1 b1; do echo "== STDERR $E"; cat "$TMP_ROOT/$E.err" 2>&1 || true; done
} > "$PROBE_OUT/diag.txt" 2>&1
# ---- END PROBE ----
...
printf "%s\n" "PROBE REACHED END OF MULTI-HOME BLOCK"
cleanup_all
exit 0
