...
test_memory_classes_bound_what_runs_side_by_side() {
  ...
  local -a huge heavy light small
  ...
  classes="$tmp/classes"
  : > "$log"
  # A fixture cost table far from every class boundary: two roots near the
  # costliest cost are huge, two at a quarter of it are heavy, four at a few
  # percent are light, and six at well under one percent are small. A root the
  # table does not list has unknown memory, so it must be treated as huge.
  huge=(tests/huge-a.sh tests/huge-b.sh tests/unlisted.sh)
  ...
  small=(tests/small-a.sh tests/small-b.sh tests/small-c.sh tests/small-d.sh tests/small-e.sh tests/small-f.sh)
  {
    ...
    printf '%s\t%s\n' tests/huge-a.sh 10000 tests/huge-b.sh 9000 tests/heavy-a.sh 3000 tests/heavy-b.sh 2500 \
  ...
  for root in "${huge[@]}" "${heavy[@]}" "${light[@]}" "${small[@]}"; do
    printf '#!/usr/bin/env bash\n:\n' > "$repo/$root"
  done
  printf '%s\n' "${huge[@]}" > "$classes/huge"
  printf '%s\n' "${heavy[@]}" > "$classes/heavy"
...
state=$FM_TEST_STATE
log() { printf '%s %s\n' "$1" "$root" >> "$FM_TEST_LOG"; }
if grep -qxF -- "$root" "$FM_TEST_CLASSES/huge" || grep -qxF -- "$root" "$FM_TEST_CLASSES/heavy"; then
  mkdir "$state/big" 2>/dev/null || log overlap
  if grep -qxF -- "$root" "$FM_TEST_CLASSES/huge"; then
    mkdir "$state/huge"
...
elif grep -qxF -- "$root" "$FM_TEST_CLASSES/light"; then
  mkdir "$state/light.$$"
...
SH
  ...
  out=$(cd "$repo" && PATH="$fakebin:$PATH" FM_LINT_JOBS=2 FM_TEST_LOG="$log" FM_TEST_CLASSES="$classes" \
    FM_TEST_STATE="$tmp/state" bin/fm-lint.sh "${small[@]}" "${light[@]}" "${heavy[@]}" "${huge[@]}" 2>&1) \
  ...
  if grep -q '^light-beside-huge ' "$log"; then
    fail "a light root ran beside a huge root"$'\n'"logged: $(cat "$log")"
  fi
  [ "$(grep '^linted ' "$log" | LC_ALL=C sort)" \
    = "$(printf 'linted %s\n' "${small[@]}" "${light[@]}" "${heavy[@]}" "${huge[@]}" | LC_ALL=C sort)" ] \
  ...
  pass "fm-lint.sh keeps costly roots apart and runs only small roots beside a huge one"
}
