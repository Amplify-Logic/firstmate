      ...
      case "$a" in
        *pane_current_command*)
          case "$mode" in
            missing) printf '%s\n' node; exit 0 ;;
            unreadable) exit 1 ;;
            # A shell-only pane reads as a dead agent until this stub's own
            # new-window relaunches into it; the relaunched pane then runs the
            # harness, which is what fm-spawn.sh's agent-up check waits for.
            zsh) if [ -e "${FM_TMUX_CALL_LOG:?}.created" ]; then printf '%s\n' codex; else printf '%s\n' zsh; fi; exit 0 ;;
            *) printf '%s\n' "$mode"; exit 0 ;;
          esac
          ;;
      esac
