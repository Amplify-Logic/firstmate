...
test_secondmate_directory_paths_are_absolute_and_output_is_stable() {
  ...
  pass "fm-brief.sh: relative inputs render stable absolute charter paths or fail loudly"
}
...
test_herdr_lab_contract_is_explicit_and_complete() {
  ...
  assert_grep "required \`--session \"\$HERDR_LAB_SESSION\"\` on every call, before any child-argv" "$brief" \
    "Herdr lab brief missing the per-call session placement contract"
...
test_secondmate_marked_request_reporting_contract() {
  ...
  assert_grep 'respond via the STATUS/ESCALATION path below, never only in this chat' "$brief" \
    "secondmate charter lost status-path return for marked requests"
  assert_grep 'This is also how you return the answer to a marked from-firstmate request above.' "$brief" \
    "secondmate charter lost marked-request answer routing through status"
