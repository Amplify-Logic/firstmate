...
test_valid_recording_and_merge_derivation() {
  local dir expected sidecar count out
  ...
  [ "$(cat "$dir/stdout")" = 'armed: state/task-a.check.sh' ] \
    || fail "GitHub arming output changed bytes"
  [ ! -s "$dir/stderr" ] || fail "GitHub arming gained diagnostic output"
  [ ! -s "$dir/glab.log" ] || fail "GitHub arming executed the GitLab CLI"
  ...
  [ "$out" = merged ] || fail "GitHub merged output changed bytes"
  grep -qxF 'pr view https://github.com/my-org/repo_name.with-dots/pull/37 --json state -q .state' "$dir/gh.log" \
    || fail "GitHub polling CLI arguments changed bytes"
  [ ! -s "$dir/glab.log" ] || fail "GitHub polling executed the GitLab CLI"
