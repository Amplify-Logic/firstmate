...
unit_quiet_entry_needs_no_posture_record() {
  ...
  rc=$?
  if [ "$rc" -ne 0 ] && [ ! -e "$st/state/.afk" ] && printf '%s' "$out" | grep -F 'an away-posture record is required' >/dev/null; then
    pass "quiet entry: an explicit away entry still requires the record"
  else
