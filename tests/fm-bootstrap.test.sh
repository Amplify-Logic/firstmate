...
#!/usr/bin/env bash
if [ "${1:-}" = --version ]; then
  printf '%s\n' "${FM_FAKE_NO_MISTAKES_VERSION:-no-mistakes version v1.59.2 (fake) 2026-06-27T00:02:18Z}"
  exit 0
fi
exit 0
SH
...
minimum no-mistakes version is accepted^no-mistakes version v1.59.2 (fake)^empty
newer no-mistakes minor is accepted^no-mistakes version v1.60.0 (fake)^empty
newer no-mistakes major is accepted^no-mistakes version v2.0.0 (fake)^empty
older no-mistakes patch reports an upgrade^no-mistakes version v1.59.1 (fake)^missing
older no-mistakes minor reports an upgrade^no-mistakes version v1.58.9 (fake)^missing
previous 1.46.0 floor reports an upgrade^no-mistakes version v1.46.0 (fake)^missing
