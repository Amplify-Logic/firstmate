...
test_dead_secondmate_escalates_without_reposting() {
  local home state corr rec status
  home=$(setup_parent dead-agent)
  state="$home/state"
  export FM_PENDING_REPLY_NOW=9500
  corr=$(fm_pending_reply_create "$home" "$state" "hibit" "answer through parent")
  fm_pending_reply_mark_delivered "$state" "$corr"
  rec=$(fm_pending_reply_path "$state" "$corr")
  status="$state/hibit.status"
  FM_PENDING_REPLY_SEND_HOOK='fail "dead agent must not receive a recovery repost"' \
    fm_pending_reply_tick_one "$state" "$corr" dead "$home/secondmate"
  [ "$(fm_pending_reply_get "$rec" phase)" = escalated ] \
    || fail "dead agent did not escalate the unresolved reply"
  assert_contains "$(cat "$status")" "pending-reply-endpoint-dead" \
    "dead-agent escalation did not name the concrete reply blocker"
  fm_pending_reply_tick_one "$state" "$corr" dead "$home/secondmate"
  [ "$(grep -c 'pending-reply-endpoint-dead' "$status")" = 1 ] \
    || fail "dead-agent escalation was published more than once"
  pass "confidently dead secondmate agents escalate once without a shell repost"
}
