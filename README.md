...
- **Two task shapes** - ship tasks deliver authorized changes; scout tasks leave standalone investigation reports when intake warrants separate research.
...
A direct launch also skips the 500k-token auto-compaction cap that the launcher applies per session for Claude Code and Codex (see [docs/configuration.md](docs/configuration.md#context-window)).
...
| `/adhd`            | Fan out divergent options for a named high-leverage decision via the ADHD CLI wrapper (`bin/fm-adhd.sh`), with bounded defaults and a mandatory ~2x cost announcement; see [docs/adhd.md](docs/adhd.md) |
| `/ahoy`            | Recap visible session outcomes since the prior real captain message plus visibly unanswered captain decisions, falling back to Bearings when invoked as the session's first real captain message |
...
- [docs/ops-command-center.md](docs/ops-command-center.md) - standing orders, watches, the tray, the private deck pane, and errands.
...
- [docs/desk-floater.md](docs/desk-floater.md) - enable the Mac always-on-top push-to-talk floater: Deepgram speech-to-text into a durable captain-input mailbox, and speak-out in the `say` voice `config/speak` names, or Deepgram Aura when it names none, each the other's fallback.
...
- [docs/kimi-harness.md](docs/kimi-harness.md) - Kimi Code worker certification record (0.27.0, 2026-07-23), the 0.31.1 primary hook re-verification (2026-08-04), and primary pointers.
