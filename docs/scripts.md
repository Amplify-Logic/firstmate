...
| `fm-fota-stage-run.sh`   | Background staging runs over a staged plan: generates the browser-side request, deduplicates by operation identity, settles `pending`/`ready`/`error`/`unknown` from an independent readback; never sends (docs/fota-staging.md) |
