...
`bin/fm-watch-arm.sh` and `bin/fm-afk-start.sh` idempotently register `bin/fm-supervision-sentinel.sh` as a per-home macOS launchd agent around entering their long-lived foreground work.
The watcher entry registers at most once per arm and only after it has observed and reported a healthy watcher.
The generated job sets `RunAtLoad`, so a bootstrap or kickstart runs a scheduled check immediately; registering before the watcher is confirmed would deliver a real `SUPERVISION DOWN` alert for the very outage that arm is ending — on every reboot, since the `gui/<uid>` agent is gone while task metadata survives.
The alarm's premise is that supervision was healthy and then stopped, so a home an arm never saw healthy has no outage to report and stays unregistered until some arm does see one.
