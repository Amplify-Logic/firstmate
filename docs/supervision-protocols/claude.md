...
9. The continuity PreToolUse gate allows wake drain, watcher arm recovery, fail-closed teardown, and the once-per-session `bin/fm-session-start.sh`, and refuses only other `bin/fm-*.sh` fleet commands while tasks are in flight and no identity-matched live watcher holds the home lock.
10. The existing turn-end guard remains the final backstop and is not replaced by this command gate; both surfaces use the canonical outage summary documented in `docs/watcher-continuity.md`.
