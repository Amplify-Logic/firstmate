Supervision host: on for this home (`config/supervision-host`; `docs/supervision-host.md` owns the design).
...
{codex}    While the record exists each checkpoint uses the longer away bound (`FM_CODEX_WATCH_CHECKPOINT_AWAY`, default 3600s, subject to the host's park cap; see `docs/supervision-host.md#the-park-boundary`), so a captain message waits until the checkpoint returns unless the captain interrupts it.
