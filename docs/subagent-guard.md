...
### Fork note on tracked settings

This Amplify-Logic fork's tracked `.claude/settings.json` also carries a fork-only `statusLine` entry upstream lacks.
The port merges the new PreToolUse registration into that file and must leave `statusLine` and the existing Bash seatbelts unchanged.
...
- `OBSERVE_ONLY_TOOLS` allows the exact names `taskoutput`, `taskstop`, `taskget`, `tasklist`, `cronlist`, `bashoutput`, and `killshell`.
...
- `PLAN_ONLY_TOOLS` allows the exact names `taskcreate` and `taskupdate`.
  These write only the harness's session-local todo list, which has no executor and cannot spawn an agent, allocate a worktree, register a schedule, or outlive the session.
  They remain separate from the observe-or-stop list because they do write, but their `task` stem is a false positive for the runnable delegation this guard owns.

Both lists match complete normalized tool names rather than substrings, so names such as `TaskCreateAgent` and `RemoteTaskCreate` remain denied.
...
It also allows `TaskCreate` and `TaskUpdate`, so the guard cannot prevent a primary from maintaining its session-local plan when no delegation power is involved.
The recommended local deny list omits those two todo tools for the same reason.
...
| --- | --- | --- |
| Claude | 16 known runnable delegation tools, listed above | Scoped guard wired and live-verified upstream; untracked local deny list verified and recommended. |
| Cursor | present (Task / agent surfaces) | Wired through the same tracked `.claude/settings.json` PreToolUse entry Claude uses; live denial of Cursor Task/Agent under matcher `.*` is **unverified** in this port. See below. |
...
### Cursor primary (this fork), wired but live-unverified for this guard

This fork runs a certified Cursor primary via `bin/fm-primary.sh cursor-grok`.
Cursor maps Claude-format `SessionStart` / `PreToolUse` / `Stop` hooks from tracked `.claude/settings.json` (see `.agents/skills/harness-adapters/SKILL.md` cursor section and `docs/arm-pretool-check.md`).
The Bash PreToolUse seatbelts in that file were live-verified on Cursor CLI `2026.07.20-8cc9c0b` in the 2026-07-22 primary lab.

This port therefore registers the delegation-shape guard in the same tracked file with matcher `.*` and `--claude`, so a Cursor primary launched from the primary home will load the hook registration.
That is not the same as live proof that Cursor fires PreToolUse for its Task/Agent tools under matcher `.*`, or that it honors the Claude-shaped deny object for those tools the way it does for Bash.
No Cursor live matrix for this guard was run in this port; do not treat the Cursor row as empirically closed.

Scoping still applies regardless of harness: `fm_primary_scope_matches` makes the script inert in a linked crewmate task worktree, so a Cursor (or Claude) worker using Task inside its disposable worktree is not denied by this script.
Crew-worktree inertness is covered by `tests/fm-subagent-pretool-check.test.sh`.
