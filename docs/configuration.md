...
Recovery keeps the pin too: the secondmate liveness relaunch ([`bin/fm-secondmate-liveness-lib.sh`](../bin/fm-secondmate-liveness-lib.sh)), used by both the session-start sweep and the watcher's liveness tick, reads `account=` back from a confidently dead secondmate's metadata and passes it as `--account`, so a pinned secondmate can never come back on a different login and refuses the relaunch rather than falling back if that account can no longer be applied.
A secondmate with no recorded `account=`, or one that `config/secondmate-harness` now respawns onto a different harness than its metadata records, is respawned with no flag, and so resolves its account as a new spawn would.
