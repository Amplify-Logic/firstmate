...
The locked mutable bootstrap path creates the visible default of `7500` estimated tokens in a primary home when the file is absent.
To select another allowance, replace the primary home's file with one positive base-10 integer followed by exactly one newline.
The next locked bootstrap convergence or `bin/fm-config-push.sh` propagates it to registered secondmates.
A secondmate does not create an independent default.
...
The stable estimate is `ceil(UTF-8 bytes / 3)` per file, a conservative local approximation rather than a provider-exact tokenizer.
An inherited `data/captain-shared.md` counts in a secondmate's total but remains primary-owned and read-only there.
The internal `/stow` skill curates only editable local files in that case and reports when the primary-owned shared file alone exceeds the allowance.
...
`fm-teardown.sh` resolves a relative `FM_HOME` and repo root the same way before its registry isolation checks, so a relative spelling cannot refuse every guarded secondmate or child home removal.
...
It uses the same live secondmate discovery and propagation helper as bootstrap, prints each live home's `crew-dispatch.json`, `crew-harness`, `backlog-backend`, `backend`, `startup-memory-budget`, and `data/captain-shared.md` result as `pushed`, `unchanged`, `skipped`, or `error`, and exits non-zero for real propagation errors or config-reread delivery failures.
...
FM_BACKEND_HERDR_IDLE_RE=  # herdr-only: empty-composer placeholder regex after shared ghost extraction plus border and prompt stripping; defaults to the fleet-wide FM_COMPOSER_IDLE_RE_DEFAULT
FM_BACKEND_HERDR_BARE_PROMPT_RE='^(❯|›)'  # herdr-only: verified agent glyphs recognized as an UNBORDERED (bare) composer row, e.g. claude's ❯ or codex's ›; alternation (not [❯›]) so LC_ALL=C grep cannot byte-match box-drawing borders; shell glyphs remain unknown rather than empty, and de-emphasised ghost/placeholder text (dim or dark-truecolor) after an agent prompt reads empty via the shared fm_composer_strip_ghost (docs/herdr-backend.md "Incident (2026-07-08)", "Incident (2026-07-10)")
...
FM_BACKEND_ORCA_IDLE_RE=  # orca-only: empty-composer placeholder regex after border/prompt stripping; defaults to the fleet-wide FM_COMPOSER_IDLE_RE_DEFAULT
...
FM_BACKEND_CMUX_IDLE_RE=  # cmux-only: empty-composer placeholder regex after border/prompt stripping; defaults to the fleet-wide FM_COMPOSER_IDLE_RE_DEFAULT
...
FM_CREW_STATE_RUNS_LIMIT=200  # recent no-mistakes run rows scanned when axi status cannot be attributed to the current code, and when a terminal failure is checked for a live run on the same branch
FM_CREW_STATE_LIVE_ROW_MAX_AGE=259200   # seconds a live run row whose head this copy cannot resolve stays trusted, measured from the run's recorded start; a backstop against a row nobody reaped, set past any plausible live run so a green PR-monitoring crew still reads working (bin/fm-crew-state.sh owns the rule)
FM_CREW_STATE_BIN=bin/fm-crew-state.sh   # test override for the current-state reader used by working/paused watcher triage
DEEPGRAM_API_KEY=       # desk voice: enables Deepgram Aura speak-out and floater speech-to-text; env or gitignored .env only, never logged (docs/desk-floater.md)
DEEPGRAM_TTS_MODEL=aura-2-thalia-en   # Deepgram Aura voice for bin/fm-deepgram-tts.sh
DEEPGRAM_STT_MODEL=nova-2   # Deepgram model for bin/fm-deepgram-stt.sh
...
FM_PI_ARM_SPAWN_GRACE_MS=12000   # milliseconds to wait for that first successor stdout before treating a silent arm as unready; defaults to FM_PI_ARM_READY_TIMEOUT_MS
...
FM_STALE_ESCALATE_SECS=240         # idle seconds before a provably-working stale pane escalates; stale panes whose crew is not provably working surface immediately unless they declare a wait (the pause verb, or an open captain-held transfer whose worker is still alive)
...
FM_COMPOSER_IDLE_RE_DEFAULT=   # fleet-wide idle-placeholder alternation covering every verified harness (grok's "Type a message...", cursor's "Add a follow-up" / "Plan, search, build anything"); the single default behind the per-backend *_IDLE_RE variables above (bin/fm-composer-lib.sh owns the value)
