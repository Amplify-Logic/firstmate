...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling or an X-mode mention, stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS`, declared external waits that remain paused past their re-surface cadence, the one-shot surface of a `captain-held:` transfer whose worker has died, and heartbeat backstop hits.
...
For an ordinary crew that has stopped, the normal-mode watcher first surfaces one stale wake, then applies that same cadence to an unchanged `paused:` endpoint only when the backend confidently reports its agent dead.
...
A verified `captain-held:` transfer is the other declared wait; `bin/fm-classify-lib.sh` owns its stream-based quiet-state, liveness, deduplication, resolution, and wedge-aging policy for both supervision modes.
...
The script header owns the exact run-head ancestry rules, including the live-over-terminal selection that keeps a replaced run's failure from being reported as the crew's current state.
That selection's unverifiable-head inference is bounded by how long ago the ledger says the run started, so a row nobody reaped eventually reports unknown rather than pinning a crew as working, while a run still monitoring a green PR is never aged out (`FM_CREW_STATE_LIVE_ROW_MAX_AGE`, docs/configuration.md).
...
In that status-log fallback, a declared wait - a `paused:` external wait or a `captain-held:` transfer - reports the distinct `paused` state with its reason.
...
The phone bridge page is a bearings glance with authenticated photo and hold-to-speak controls, plus a private `/deck` page rendering the Action Deck's `fm-deck.v1` model, served on loopback and published with Tailscale Serve; see [`bridge-view.md`](bridge-view.md).
...
On `attached` it stays live across identity-matched successors, and a cycle that ends with no reason line and no healthy successor is resolved against the watcher's identity-bound delivery record before becoming the typed nonzero `watcher: FAILED - cycle ended without an actionable reason` result.
...
On macOS, `bin/fm-supervision-sentinel.sh` adds a per-home launchd check outside the harness process tree and reuses the active channels in [`wedge-alarm.md`](wedge-alarm.md) when the watcher lock or beacon becomes unhealthy.
...
The watcher and daemon share `bin/fm-classify-lib.sh` for captain-relevant status verbs, declared-wait vocabulary, status-scan primitives, and the declared-pause recheck cadence and grouping fold.
...
Because that recheck fold has one owner, both modes surface `paused:` waits that share one blocking reason as a single item rather than one per task, and a pause that names an explicit captain decision uses the longer `FM_PAUSE_CAPTAIN_RESURFACE_SECS` cadence because it cannot clear until the captain acts.
...
`fm-watch.sh` polls each window's backend for a busy state: tmux, zellij, orca, and cmux have no native primitive and always report unknown, preserving the original pane-tail-regex detection unchanged; herdr's `agent.get` semantic state (working/idle/done/blocked) is consulted first for stale detection, trusting only a bare `busy` verdict outright - `idle` and unknown verdicts are corroborated against the same regex (herdr can read idle/blocked while a harness is mid-turn; [herdr-backend.md](herdr-backend.md) owns the incidents and evidence).
...
For capable herdr sessions, the same watcher replaces its terminal sleep with a bounded native event wait that immediately surfaces `blocked` (after busy-footer corroboration, so a mid-turn worker is not escalated); [herdr-backend.md](herdr-backend.md#native-paneagent_status_changed-push-escalation-immediate-blocked-wake) owns the mechanism, capability gates, and verification evidence.
[`watcher-continuity.md`](watcher-continuity.md#filesystem-event-continuity) owns the glasses mailbox and bridge-inbox filesystem-event behavior across live waits and watcher-cycle boundaries.
...
`fm-spawn.sh` is the other consumer of those classifiers: after submitting the launch it waits, on a hard bound, for an agent to actually own the new endpoint before it reports the spawn as started or types a post-launch brief into the pane.
A pane still proven to be a bare shell at that bound refuses loudly, because until an agent owns the pane everything typed there is shell input, which is how a brief used to spill silently into a dead shell that still looked alive.
A backend with no liveness reader at all (zellij, Orca, cmux) cannot run the check, so its spawns proceed exactly as they did before rather than being refused on a signal that was never available, because an unsupported check must never remove a capability that already worked.
An inconclusive read is tolerated the same way only where the prompt already rode the launch line, which keeps a harness whose process cannot be attributed from outside the pane (`pi` on tmux, [`docs/tmux-backend.md`](tmux-backend.md)) behaving as before, while the path that still has to type the brief demands positive proof instead.
What that launch line carries as its prompt is a backend decision rather than a harness one: every backend sends the whole encoded brief inline, except herdr, where it sends a one-line pointer at the brief file because pasting a multi-kilobyte multi-line brief through a pane shell there reliably failed to start the agent at all ([herdr-backend.md](herdr-backend.md) owns that incident and its evidence).
A structurally gone endpoint is the opposite case and refuses on the first read rather than waiting out the bound, and its recovery is a re-spawn rather than a relaunch into a pane that no longer exists.
...
Capability evidence from `data/capability-outcomes.log` layers on those cost-filtered profiles only (`docs/configuration.md` "Capability outcome log"); it never invents a harness outside the matched rule's `use` set.
...
For cursor, which has no effort flag, `fm-spawn.sh` instead folds the effort axis into the model id (see the harness-adapters launch-profile table).
...
The parent guards every marked request against a missing correlated report without reading the secondmate conversation.
`bin/fm-pending-reply-lib.sh` owns correlation, one automatic repost request, liveness-aware dead-agent escalation, and retained resolution records.
...
A squash merge also supplies its own commit body, because GitHub's default squash message reintroduces agent co-author trailers from the PR's commit list even when the branch tip carries none; the helper's header owns how that message is read and sanitized.
A well-formed GitLab merge request URL can be monitored as documented in [`docs/gitlab-merge-watch.md`](gitlab-merge-watch.md), but the GitHub-only merge helper explicitly refuses it rather than sending it to the wrong forge.
