...
A backend or harness whose liveness cannot be read is left to behave exactly as before rather than refused on a signal that was never available, because an unsupported check must never remove a capability that already worked.
