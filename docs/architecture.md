...
A record written before the endpoint-binding and incarnation fields existed can name neither an isolated copy nor an endpoint it can prove is its own, which left it with no supported retirement path at all.
`--legacy-record` reconciles exactly those two degradations: an absent worktree identity becomes nothing to return to the pool, and an unbound endpoint becomes nothing to stop, the latter only once the backend's own liveness read reports that endpoint absent rather than merely agent-less.
Every dirty, unlanded-work, ambiguous-field, and live-endpoint refusal still applies unchanged.
[`bin/fm-teardown.sh`](../bin/fm-teardown.sh)'s header owns the landed-work proofs, the legacy-record reconciliation rules, slot-ownership proof, PR-discovery fallback, pre-teardown run conclusion, and stale-lock recovery procedure; [`tests/fm-teardown-endpoint-safety.test.sh`](../tests/fm-teardown-endpoint-safety.test.sh) and [`tests/fm-secondmate-safety.test.sh`](../tests/fm-secondmate-safety.test.sh) pin the slot-collision boundary.
