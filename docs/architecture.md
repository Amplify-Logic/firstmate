...
The phone bridge page is a bearings glance with authenticated photo and hold-to-speak controls, served on loopback and published with Tailscale Serve; see [`bridge-view.md`](bridge-view.md).
...
When a home has a glasses mailbox or bridge inbox, that same terminal wait is also interrupted by a filesystem change so the next authenticated check sweep runs immediately; `bin/fm-file-event-lib.sh` owns the path list and wait contract.
