...
When glasses watch paths also exist, the watcher races this wait against the bounded file-event wait owned by `bin/fm-file-event-lib.sh`; a file event interrupts the cycle without being counted as a Herdr connection failure or disabling this fast path.
