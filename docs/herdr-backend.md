...
`bin/fm-watch.sh` splices this in as the watcher's terminal wait (`event_wait_or_sleep`, replacing the blind `sleep POLL` for push-capable homes; the fork reaches it through the guarded either/or of hook W1): on a returned `blocked` it maps `pane_id -> <session>:<pane_id> -> task`, exempts `kind=secondmate` endpoints and a declared wait (`bin/fm-classify-lib.sh`'s `status_declared_wait`, which owns the verb set and the fold that decides a hold is still open; an idle composer after a captain hold is expected, not a surprise wait-on-human), corroborates against the rendered busy footer (so a mid-turn worker is not treated as waiting on human), and enqueues an immediate `stale` wake only when the pane is truly blocked.
