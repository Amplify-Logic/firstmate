...
.env                 optional Relay pairing token (presence-gates section 14), mail-plane credentials (schema: docs/configuration.md "Mail plane"), typed dispatch resolution key TYPESAFE_API_KEY (presence-gates bin/fm-dispatch-resolve.sh; docs/configuration.md "Typed dispatch resolution"), and optional DEEPGRAM_API_KEY for desk voice (docs/desk-floater.md); LOCAL, gitignored; never logged
...
config/desk-floater-signing-identity  optional codesigning identity for the desk floater, or "-" for ad-hoc; LOCAL, gitignored, not inherited; absent, or missing, expired or failing to sign, uses the first "Apple Development" identity, else ad-hoc, with a note and never a stopped launch; see docs/configuration.md "Desk floater signing identity"
...
state/               runtime records and signals; gitignored
  ...
  <id>.visible-label .visible-workspace-* .visible-status-all.lock  the last Herdr labels published for a task and a project workspace, and the single-flight lock of a refresh pass; presentation caches only, safe to delete, and owned by bin/fm-visible-status.sh
  ...
  desk-voice/        durable desk-floater push-to-talk fallback mailbox, used when a message (a transcript, screenshot paths, or both) cannot be typed into the primary chat: inbox/ holds pending captain-input transcripts, processed/ holds drained ones, shots/ holds the newest floater screenshots; `bin/fm-desk-voice.sh drain` empties the inbox (docs/desk-floater.md)
  .shift .shift-log .shift-mailbox-outage   armed glasses-shift record, its plain timestamped event log, and the self-check's outage episode marker; written by bin/fm-shift.sh start, stop, and its registered fm-shift check; stop removes `.shift` and `.shift-mailbox-outage` and leaves the append-only `.shift-log`, which spans shifts; never touch; see docs/shift-loop.md
  ...
  .pr-check-quarantine/ .pr-check-migration.log .pr-check-migration-scan-v1   private non-runnable storage, per-task outcomes, and completion markers for the non-executing legacy PR-check migration
  ...
  speak-voice-confirmed .speak.lock .speak.lock.owner.* .speak.lock.steal speak-muted speak-history/ .speak-audio.* speak-stop-generation .speak-player  desk voice-out's confirmed-voice memory, the floater's mute flag, bounded recent-reply history with its kept audio (and that audio in transit), stop counter and playing-player record, the per-home playback lock the detached speaker holds so a second line waits instead of overlapping, its owner directories, and the transient steal mutex; a speaker killed mid-line leaves the lock and its owner directory behind and the next speaker reclaims them; bin/fm-speak.sh's header owns the mechanic
  .reply-speak-stop .reply-speak-seen   the Claude reply-speak Stop hook's latest-Stop token and last accounted spoken line; bin/fm-claude-reply-speak.sh's header owns the mechanic
...
The verified harnesses are `claude`, `codex`, `opencode`, `pi`, `pi-signed`, `grok`, `kimi`, `cursor`, and `omp`, plus `muse`, `gemini`, `rovo`, `agy`, `devin`, and this fork's `prime-agent` for crewmates and scouts only; never dispatch on an unverified adapter.
...
That skill owns registry syntax, secondmate-scope intake, delivery-mode selection, outward-facing consent, clone and initialization procedure, safe rollback, and removal preflight.
...
- `daily-todo-freshness` - load before producing or updating any captain-facing to-do, needs-you, or waiting-on-you surface, and before claiming in chat that anything is waiting on the captain.
