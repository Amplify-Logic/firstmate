...
state/               runtime records and signals; gitignored
  ...
  speak-voice-confirmed .speak.lock .speak.lock.owner.* .speak.lock.steal  desk voice-out's confirmed-voice memory, the per-home playback lock the detached speaker holds so a second line waits instead of overlapping, its owner directories, and the transient steal mutex; a speaker killed mid-line leaves the lock and its owner directory behind and the next speaker reclaims them; bin/fm-speak.sh's header owns the mechanic
...
- `usage-restoration` - load when a partner asks for a system's usage to be restored, when a system's usage, filter or service counters drop to zero or to an obviously wrong value, or when a system reports `A.13 storage file corrupted`.
